...
void Robot::lights(int middle_power, int left_color, int left_intensity) {
  uint8_t d[8];
...
void Robot::blink_led() {          // every 400ms
  static int t = 0;
  if (t < 20)
...
void Robot::motor_raw(double forward_speed, double angular_speed) {  
...
void Robot::move(double forward_speed) {
  if (playerc_position2d_set_cmd_vel(position2d, forward_speed, 0, 0, 1))
    quit("move command failed: playerc_position2d_set_cmd_vel");
}

void Robot::turn(double forward_speed, double angular_speed) {
  if (playerc_position2d_set_cmd_vel(position2d, forward_speed, 0, angular_speed, 1))
    quit("move command failed: playerc_position2d_set_cmd_vel");
...
void Robot::calibrate_ir() {
  // turn in place (+360 degrees, then -360 degrees)
  printf("Calibratinng IR sensors...");
  ir_values readings;
  ...
  printf("Place Create at starting point, and press Enter key when ready.\n");
  getchar();
}

Robot::ir_values Robot::read_ir_raw() {
  ir_values rtn;
...
Robot::ir_values Robot::read_ir() {       // returnes calibrated values
  ir_values rtn;
  playerc_client_read(client);
...
void Robot::quit(const char *msg) {
  printf("%s", msg);
  shutdown();
  exit(1);
}
