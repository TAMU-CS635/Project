...
create_parse_sensor_packet(create_comm_t* r, unsigned char* buf, size_t buflen)
{
  ...
  memcpy(&unsigned_int, buf+idx, 2);    // Wall Signal
  idx += 2;
  r->wall_signal = ntohs(unsigned_int);
  
  memcpy(&unsigned_int, buf+idx, 2);    // Cliff Left Signal
  idx + =2;
  r->cliff_left_signal = ntohs(unsigned_int);
  
  memcpy(&unsigned_int, buf+idx, 2);    // Cliff Front Left Signal
  idx += 2;
  r->cliff_front_left_signal = ntohs(unsigned_int);
  
  memcpy(&unsigned_int, buf+idx, 2);    // Cliff Front Right Signal
  idx += 2;
  r->cliff_front_right_signal = ntohs(unsigned_int);
  
  memcpy(&unsigned_int, buf+idx, 2);    // Cliff Right Signal
  idx += 2;
  r->cliff_right_signal = ntohs(unsigned_int);
  
  idx++;   // Cargo Bay Digital
  idx+=2;   // Cargo Bay Analog

  r->charging_sources = buf[idx++];
