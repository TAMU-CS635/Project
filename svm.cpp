...
int main()
{
    ...
    for(int n = 0; n < left_sample; n++){
        ...
        img_matbefore = imread(file,0); // I used 0 for greyscale
        Size size(640,480);
        resize(img_matbefore,img_mat,size);
    ...
    for(int n = 0; n < left_mid_sample; n++){
        ...
        img_matbefore = imread(file,0); // I used 0 for greyscale
        Size size(640,480);
        resize(img_matbefore,img_mat,size);        ii = 0; // Current column in training_mat
        for (int i = 0; i<img_mat.rows; i++) {
            for (int j = 0; j < img_mat.cols; j++) {
                training_mat.at<float>(counter,ii++) = img_mat.at<uchar>(i,j);
            }
